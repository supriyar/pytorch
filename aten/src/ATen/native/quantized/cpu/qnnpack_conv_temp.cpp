...
class QNNPACKConv2d final : public c10::OperatorKernel {
 public:
  Tensor operator()(
    ...
    TORCH_CHECK(!bias.defined() || (bias.ndimension() == 1 && bias.size(0) == out_ch),
        "qnnpack_conv2d(): Given weight of size ", weight.sizes(),
        ", expected bias to be 1-dimensional with ", out_ch, " elements",
    ...
    float outScale = output.q_scale();
    int outZeroPoint = output.q_zero_point();
    ...
    const qnnp_status createStatus = qnnp_create_convolution2d_nhwc_q8(
            ...
            act.q_zero_point(), /* input zero_point */
            act.q_scale(),   /* input scale */
            ...
            0, //activationLimits(outScale, outZeroPoint, Ac).first, /* output min */
            255,//activationLimits(outScale, outZeroPoint, Ac).second, /* output max */
            0 /* flags */,
            &qnnpackOperator_);
...
static auto registry = torch::RegisterOperators().op(
    "quantized::qnnpack_conv2d_temp",
    torch::RegisterOperators::options().kernel<QNNPACKConv2d>(
