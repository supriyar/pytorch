...
class TestQNNPackOps(TestCase):
    ...
    def test_qnnpack_linear(self, output_channels, X):
        ...
        W_pack = torch.ops.quantized.qnnpack_linear_prepack(W_q, b_q)

        Y_q = torch.ops.quantized.qnnpack_linear_op(X_q, W_pack, Y_scale, Y_zp)
    ...
    def test_qnnpack_conv(
        ...
        W_pack = torch.ops.quantized.qnnpack_prepack_conv(W_q, b_q, stride, padding, dilation, groups)
        qconv = torch.ops.quantized.qnnpack_conv2d
        if use_relu:
            qconv = torch.ops.quantized.qnnpack_conv2d_relu
    ...
    @given(output_channels=st.sampled_from([2, 4, 5, 8, 16, 32]),
           X=hu.tensor(shapes=hu.array_shapes(2, 3, 8, 15),
                       qparams=hu.qparams(dtypes=torch.quint8)))
    def test_qnnpack_linear2(self, output_channels, X):
        X, (X_scale, X_zp, torch_type) = X
        qmin = torch.iinfo(torch_type).min
        qmax = torch.iinfo(torch_type).max
        ...
        # Assert equal
        np.testing.assert_array_almost_equal(Y_q_ref2.dequantize().numpy(), Y_q.dequantize().numpy(), decimal=0)
